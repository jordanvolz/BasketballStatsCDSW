...
for (stat <- statArray){
  ...
  val trainData = spark.sql("select name, year, exp, mp, " + stat + "0," + stat + "  as label from basketball.ml where year<2018")
  val testData = spark.sql("select name, year, exp, mp, " + stat + "0," + stat + "  as label from basketball.ml where year=2018")
